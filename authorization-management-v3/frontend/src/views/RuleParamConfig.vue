<template>
  <div class="rule-param-page">
    ...
    <div class="table-card">
      <table class="data-table">
        ...
        <tbody>
          ...
          <tr v-else-if="tableData.length === 0"><td colspan="10" style="text-align: center; padding: 40px; color: #909399;">暂无数据</td></tr>
          <tr v-else v-for="(row, index) in tableData" :key="row.id">
            ...
            <td class="col-action"><span class="icon-btn" title="编辑" @click="openDialog(row)">✏️</span></td>
            <td class="col-name">{{ row.name }}</td>
            <td class="col-name-en">{{ row.nameEn }}</td>
            <td class="col-status"><span class="status-tag" :class="row.status === 'ACTIVE' ? 'status-active' : 'status-inactive'">{{ row.status === 'ACTIVE' ? '生效' : '失效' }}</span></td>
                ...
                <div class="dialog-form-item">
                  ...
                  <div class="select-wrapper">
                    ...
                    <div class="select-dropdown" v-show="activeDropdown === 'businessObject' + index">
                      <div class="select-option" v-for="item in businessObjectOptions" :key="item.value" @click="selectBusinessObject(index, item.value)">{{ item.label }}</div>
                    </div>
                  </div>
                </div>
                <div class="dialog-form-item">
              ...
              <div class="dialog-form-item">
                ...
                <div class="select-wrapper">
                  ...
                  <div class="select-dropdown" v-show="activeDropdown === 'dataType'">
                    <div class="select-option" v-for="item in dataTypeOptions" :key="item.value" @click="selectDataType(item.value)">{{ item.label }}</div>
                  </div>
                </div>
              </div>
            </div>
    ...
    <!-- 消息提示 -->
    <div class="message-box" v-if="message.show" :class="'message-' + message.type">{{ message.text }}</div>
...
async function request(url, options = {}) {
  const config = { headers: { 'Content-Type': 'application/json' }, ...options, headers: { ...options.headers, 'Content-Type': 'application/json' } }
...
const api = {
  getBusinessObjects: () => request('/lookup/business-objects'),
  getRuleParamList: (params) => {
    const queryStr = Object.entries(params).filter(([, v]) => v !== null && v !== undefined && v !== '').map(([k, v]) => `${k}=${encodeURIComponent(v)}`).join('&')
    return request(`/rule-params?${queryStr}`)
  },
  getRuleParamDetail: (id) => request(`/rule-params/${id}`),
...
export default {
  ...
  data() {
    return {
      loading: false, saving: false, activeDropdown: '', selectAll: false, selectedRows: [], jumpPage: 1,
      message: { show: false, type: 'info', text: '' },
      confirmDialog: { show: false, text: '', onConfirm: () => {}, onCancel: () => {} },
      queryParams: { name: '', nameEn: '', status: '' },
      pagination: { pageNum: 1, pageSize: 10, total: 0 },
      statusOptions: [{ value: 'ACTIVE', label: '生效' }, { value: 'INACTIVE', label: '失效' }],
      businessObjectOptions: [],
      dataTypeOptions: [{ value: 'TEXT', label: '文本' }, { value: 'NUMBER', label: '数值' }, { value: 'COMPARE_FIELD', label: '比对字段' }, { value: 'RATIO', label: '比率' }],
      tableData: [],
      dialogVisible: false, editingRow: null,
      formData: { name: '', nameEn: '', businessRows: [{ businessObject: '', valueLogic: '' }], isActive: null, dataType: '' }
    }
  },
  computed: { totalPages() { return Math.ceil(this.pagination.total / this.pagination.pageSize) || 1 } },
  mounted() { document.addEventListener('click', this.handleClickOutside); this.loadLookupData(); this.loadTableData() },
  ...
  methods: {
    ...
    getStatusLabel(value) { const item = this.statusOptions.find(s => s.value === value); return item ? item.label : '' },
    getBusinessObjectLabel(value) { const item = this.businessObjectOptions.find(s => s.value === value); return item ? item.label : value },
    getDataTypeLabel(value) { const item = this.dataTypeOptions.find(s => s.value === value); return item ? item.label : value },
    selectBusinessObject(index, value) { this.formData.businessRows[index].businessObject = value; this.activeDropdown = '' },
    selectIsActive(value) { this.formData.isActive = value; this.activeDropdown = '' },
    selectDataType(value) { this.formData.dataType = value; this.activeDropdown = '' },
    addBusinessRow() { this.formData.businessRows.push({ businessObject: '', valueLogic: '' }) },
    removeBusinessRow(index) { this.formData.businessRows.splice(index, 1) },
    handleSelectAll() { this.selectedRows = this.selectAll ? this.tableData.map(row => row.id) : [] },
    ...
    handleJumpPage() { if (this.jumpPage >= 1 && this.jumpPage <= this.totalPages) { this.pagination.pageNum = this.jumpPage; this.loadTableData() } },
    handleQuery() { this.pagination.pageNum = 1; this.loadTableData() },
    checkSelection() { if (this.selectedRows.length === 0) { this.showMessage('请先选择数据', 'warning'); return false } return true },
    ...
    openDialog(row = null) {
      this.editingRow = row
      if (row) {
        this.formData.name = row.name; this.formData.nameEn = row.nameEn
        this.formData.businessRows = row.businessMappings && row.businessMappings.length > 0 ? row.businessMappings.map(m => ({ businessObject: m.businessObject, valueLogic: m.valueLogic })) : [{ businessObject: '', valueLogic: '' }]
        this.formData.isActive = row.status === 'ACTIVE'; this.formData.dataType = row.dataType || ''
      } else {
        this.formData = { name: '', nameEn: '', businessRows: [{ businessObject: '', valueLogic: '' }], isActive: null, dataType: '' }
      }
      this.dialogVisible = true
    },
    closeDialog() { this.dialogVisible = false; this.editingRow = null },
    async handleSave() {
      if (!this.formData.name) { this.showMessage('请输入名称', 'warning'); return }
      if (!this.formData.nameEn) { this.showMessage('请输入名称英文', 'warning'); return }
      if (this.formData.isActive === null) { this.showMessage('请选择是否生效', 'warning'); return }
      if (!this.formData.dataType) { this.showMessage('请选择数据类型', 'warning'); return }
      this.saving = true
      try {
    ...
    showConfirm(text) { return new Promise((resolve) => { this.confirmDialog = { show: true, text, onConfirm: () => { this.confirmDialog.show = false; resolve(true) }, onCancel: () => { this.confirmDialog.show = false; resolve(false) } } }) },
    handleClickOutside(event) { if (!event.target.closest('.select-wrapper')) this.activeDropdown = '' }
  }
}
</script>
...
.rule-param-page { padding: 20px; background-color: #f5f7fa; min-height: 100vh; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; font-size: 14px; color: #333; }
...
.col-name-en { min-width: 140px; }
.col-user { width: 100px; text-align: center; }
...
.modal-dialog { background: #fff; border-radius: 8px; min-width: 600px; box-shadow: 0 2px 12px rgba(0, 0, 0, 0.2); }
...
.dialog-form-row { display: grid; grid-template-columns: 1fr 1fr 40px; gap: 16px; align-items: center; }
.dialog-form-row:last-child { grid-template-columns: 1fr 1fr; }
.dialog-form-item { display: flex; align-items: center; }
.dialog-form-label { width: 80px; text-align: right; color: #666; flex-shrink: 0; margin-right: 12px; }
.dialog-form-label.required::before { content: '*'; color: #f56c6c; margin-right: 4px; }
.business-rows { display: flex; flex-direction: column; gap: 12px; padding: 12px; background: #fafafa; border-radius: 4px; border: 1px solid #e4e7ed; }
.add-btn-wrapper { display: flex; align-items: center; gap: 8px; }
.add-btn, .remove-btn { width: 28px; height: 28px; border-radius: 4px; display: flex; align-items: center; justify-content: center; cursor: pointer; font-size: 18px; }
.add-btn { background: #409eff; color: #fff; }
.add-btn:hover { background: #66b1ff; }
.remove-btn { background: #f56c6c; color: #fff; }
