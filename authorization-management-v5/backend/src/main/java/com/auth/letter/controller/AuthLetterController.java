 ...
 * REST API endpoints for authorization letter management
 */
@Component
...
public class AuthLetterController {
     ...
     * PUT /api/auth-letters/{id}/expire
     */
    @PUT
    @Path("/{id}/expire")
    public Response expire(@PathParam("id") Long id) {
        try {
    ...
    /**
     * Save and publish authorization letter
     * PUT /api/auth-letters/{id}/save-and-publish
     */
    @PUT
    @Path("/{id}/save-and-publish")
    public Response saveAndPublish(@PathParam("id") Long id, AuthLetterDetailDTO dto) {
        try {
            authLetterService.update(id, dto);
            authLetterService.publish(id);
            return Response.ok(ApiResponse.success("保存并发布成功")).build();
        } catch (RuntimeException e) {
            return Response.ok(ApiResponse.error(400, e.getMessage())).build();
        }
    }
    ...
    public Response getOperationLogs(
            ...
            @QueryParam("pageNum") @DefaultValue("1") Integer pageNum,
            @QueryParam("pageSize") @DefaultValue("10") Integer pageSize) {
        PageResult<OperationLogVO> result = authLetterService.getOperationLogs(id, pageNum, pageSize);
        return Response.ok(ApiResponse.success(result)).build();
    }
}