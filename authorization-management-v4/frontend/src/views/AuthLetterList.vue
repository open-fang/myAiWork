      ...
      <table class="data-table">
        ...
        <tbody>
          ...
          <tr v-else v-for="(row, index) in tableData" :key="row.id">
            ...
            <td class="col-level">{{ row.authTargetLevelText || '-' }}</td>
            <td class="col-region">{{ row.applicableRegionText || '-' }}</td>
            <td class="col-level">{{ row.authPublishLevelText || '-' }}</td>
            <td class="col-org">{{ row.authPublishOrgText || '-' }}</td>
...
.auth-letter-list-page { padding: 20px; background-color: #f5f7fa; min-height: calc(100vh - 50px); font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; font-size: 14px; color: #333; }
...
.message-box { position: fixed; top: 70px; left: 50%; transform: translateX(-50%); padding: 12px 24px; border-radius: 4px; z-index: 1000; }
