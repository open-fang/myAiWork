...
| ✅ 运行测试并收集结果 | ❌ 修复编译错误（属于开发职责） |
| ✅ 记录测试问题到 `docs/test-issues.md` | ❌ 修改 pom.xml 添加依赖（需通知后端开发） |
| ✅ 编写测试报告 | ❌ 修改数据库迁移脚本 |
| ✅ 提供修复建议 | ❌ 直接修改任何非测试文件 |
...
# 1. 记录当前运行状态（用于 Teardown 还原）
echo "记录当前服务状态..."
ORIGINAL_SERVICES=""
pgrep -f "java.*authorization" && ORIGINAL_SERVICES="$ORIGINAL_SERVICES backend" || true
pgrep -f "python.*http.server.*8080" && ORIGINAL_SERVICES="$ORIGINAL_SERVICES frontend" || true
docker ps --filter "name=postgres" -q && ORIGINAL_SERVICES="$ORIGINAL_SERVICES postgres" || true
echo "原有服务: $ORIGINAL_SERVICES"
...
| **E2E自动化测试** | `npx playwright test` | 如有配置则执行 |
...
```bash
# ============================================
# Phase 3: Teardown - 还原环境
# ============================================

echo "=== Phase 3: Teardown - 还原环境 ==="
...
| 阶段 | 步骤 | 检查项 |
|------|------|--------|
| **Setup** | 1. 记录原有服务状态 | ✅ 记录 ORIGINAL_SERVICES |
| | 2. 停止已存在服务 | ✅ 杀掉后端、前端进程 |
| | 3. 拉取最新代码 | ✅ git pull |
| | 4. 启动数据库 | ✅ Docker PostgreSQL |
| | 5. 编译并启动后端 | ✅ mvn spring-boot:run |
| | 6. 启动前端 | ✅ python http.server |
| | 7. 验证环境就绪 | ✅ curl 检查端口 |
| **Execute** | 1. 确认测试用例已评审 | ✅ 检查 docs/test-cases.md |
| | 2. 执行后端单元测试 | ✅ mvn test |
| | 3. 执行 API 接口测试 | ✅ 按用例文档执行 |
| | 4. 执行 E2E 测试 | ✅ playwright test（如有） |
| | 5. 记录测试结果 | ✅ 通过/失败/截图 |
...
### Phase 1: Setup 检查清单
- [ ] 记录原有服务状态（ORIGINAL_SERVICES）
- [ ] 停止已存在的后端服务
- [ ] 停止已存在的前端服务
- [ ] 拉取最新代码（git pull）
- [ ] 启动/检查数据库（Docker PostgreSQL）
