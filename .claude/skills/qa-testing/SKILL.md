...
│  3. 运行单元测试   → mvn test（必须100%通过）                 │
│  4. 启动后端服务   → mvn spring-boot:run                     │
│  5. 执行API测试    → 测试核心接口                             │
│  6. 记录问题       → 发现问题记录到 docs/test-issues.md       │
│  7. 通知修复       → 通知对应开发角色                         │
└─────────────────────────────────────────────────────────────┘
```
...
1. **记录问题**：详细记录到 `docs/test-issues.md`
2. **通知开发**：通过 SendMessage 通知相关开发角色
3. **等待修复**：不要自行修改功能代码
4. **重新验证**：开发修复后重新执行回归测试
...
# 0. 【重要】Java版本兼容性检查
echo "检查Java版本兼容性..."
PROJECT_JAVA_VERSION=$(grep -oP '(?<=<java.version>)[^<]+' pom.xml 2>/dev/null || echo "未找到")
CURRENT_JAVA_VERSION=$(java -version 2>&1 | head -1 | cut -d'"' -f2 | cut -d'.' -f1)

echo "项目要求Java版本: $PROJECT_JAVA_VERSION"
echo "当前环境Java版本: $CURRENT_JAVA_VERSION"

# 警告版本不匹配
if [ "$PROJECT_JAVA_VERSION" = "1.8" ] && [ "$CURRENT_JAVA_VERSION" != "1" ]; then
    echo "⚠️ 警告: 项目要求Java 8，但当前环境是Java $CURRENT_JAVA_VERSION"
    echo "⚠️ 这可能导致编译成功但运行失败，或使用了不兼容的API（如List.of）"
fi
...
**历史教训**：项目要求Java 8，但开发环境使用Java 11+，导致使用了Java 9+ API（如`List.of()`），代码在开发环境编译通过，但在目标Java 8环境编译失败。

### Java 9+ 不兼容API清单

以下API在Java 8中**不可用**，测试时必须检查：

| API | Java版本引入 | Java 8替代方案 |
|-----|-------------|---------------|
| `List.of()` | Java 9 | `Arrays.asList()` |
| `Set.of()` | Java 9 | `new HashSet<>(Arrays.asList())` |
| `Map.of()` | Java 9 | `Collections.singletonMap()` 或手动构建 |
| `String.isBlank()` | Java 11 | `str.trim().isEmpty()` |
...
```bash
# 在项目根目录执行，扫描不兼容的Java 9+ API
echo "=== Java 8兼容性扫描 ==="

# 扫描 List.of / Set.of / Map.of
INCOMPATIBLE=$(grep -rn "List\.of\|Set\.of\|Map\.of\|\.isBlank()" \
    --include="*.java" \
    --exclude-dir=target \
    --exclude-dir=test \
...
if [ -n "$INCOMPATIBLE" ]; then
    echo "❌ 发现Java 9+不兼容API："
    echo "$INCOMPATIBLE"
    echo ""
    echo "请替换为Java 8兼容写法！"
    exit 1
else
    echo "✅ 未发现Java 9+不兼容API"
fi
```
...
```bash
# 使用Maven强制按Java 8字节码编译
mvn clean compile -Dmaven.compiler.source=1.8 -Dmaven.compiler.target=1.8

# 检查编译结果
if [ $? -ne 0 ]; then
    echo "❌ 编译失败，可能存在Java版本兼容性问题"
    exit 1
fi
```

#### 方法三：Animal Sniffer插件（推荐用于CI）
...
- [ ] 检查pom.xml中`<java.version>`配置
- [ ] 确认当前环境Java版本与项目要求一致
- [ ] 执行静态代码扫描，检查Java 9+ API
- [ ] 使用目标版本编译验证
- [ ] 记录版本差异警告到测试报告
...
### Phase 1: Setup 检查清单
- [ ] **检查Java版本兼容性（项目要求版本 vs 当前环境版本）**
- [ ] **执行Java API兼容性测试（检测Java 9+ API在Java 8环境）**
- [ ] 停止后端服务
- [ ] 停止前端服务
- [ ] 停止并删除数据库容器
- [ ] 拉取最新代码（git pull）
