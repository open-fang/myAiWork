...
后端开发工程师**必须**在提交代码前完成以下自测，**严禁跳过任何步骤**：
...
│  2. 测试验证    → mvn test（必须100%通过）                    │
│  3. API验证     → 启动服务，手动测试接口                       │
│  4. 兼容性检查  → 扫描Java 9+ API                            │
│  5. DTO一致性   → 检查请求/响应DTO与前端匹配                   │
└─────────────────────────────────────────────────────────────┘
```

### 低级错误禁止清单

以下错误属于**低级错误**，出现将被退回并记录：

| 错误类型 | 示例 | 影响 |
|----------|------|------|
| **类型不匹配** | DTO是String，前端发Array | 接口500错误 |
| **Java版本不兼容** | 使用List.of()（Java 9+） | Java 8编译失败 |
| **测试编译失败** | 测试代码类型错误 | 无法运行测试 |
| **API方法错误** | 定义POST但用PUT测试 | 接口404/405错误 |
| **SQL语法错误** | 表名错误、字段歧义 | 运行时异常 |
| **NullPointerException** | Boolean三元表达式未处理null | 运行时异常 |
...
```bash
# 完整的自测命令（必须全部通过）
mvn clean compile && mvn test && echo "✅ 自测通过，可以提交"
```
